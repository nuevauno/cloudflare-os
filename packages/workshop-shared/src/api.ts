...
export interface PosPartnerView { id:string; kind:'person'|'company'; displayName:string; email?:string; phone?:string; taxIdentifier?:string; street?:string; street2?:string; city?:string; regionCode?:string; postalCode?:string; countryCode:string; website?:string }
export type PosPartnerInputView = Pick<PosPartnerView,'displayName'> & Partial<Omit<PosPartnerView,'id'|'displayName'>>;
export type PosOperatorRoleView = 'manager'|'cashier'|'minimal';
export interface PosOperatorView {id:string;displayName:string;role:PosOperatorRoleView;hasPin:boolean;linkedToCurrentIdentity:boolean}
export interface PosCashDenominationView {denominationMinor:number;quantity:number}
export interface PosSessionPaymentCountView {paymentMethodId:string;countedMinor:number}
export interface PosCatalogCategoryView {id:string;name:string;parentId?:string;status:'active'|'archived'}
export interface PosCatalogTaxView {id:string;name:string;rateBasisPoints:number;priceIncluded:boolean}
export interface PosComboComponentInputView {id?:string;name:string;categoryId?:string;quantity:number;minChoices:number;maxChoices:number;items:Array<{id?:string;productVariantId:string;extraPriceMinor:number;maxFreeQuantity:number;sequence:number}>}
export interface PosVariantAttributeInputView {attributeId?:string;attributeName:string;valueId?:string;valueName:string;priceExtraMinor:number;color?:string}
export type PosVariantCreationModeView='always'|'dynamic'|'no_variant'
export interface PosTemplateAttributeLineInputView {id?:string;attributeId?:string;attributeName:string;createVariant:PosVariantCreationModeView;values:Array<{valueId?:string;valueName:string;priceExtraMinor:number;color?:string}>}
export interface PosPriceRuleView {priceListId:string;priceListName?:string;priceMinor:number;minQuantity:number;validFrom?:string;validTo?:string}
export interface PosLotInputView {id?:string;name:string;tracking:'lot'|'serial';quantityMilli:number;expirationDate?:string}
export interface PosCatalogProductInputView {id?:string;templateId?:string;name:string;description?:string;sku?:string;barcode?:string;categoryId?:string;priceMinor:number;taxId?:string;kind:'product'|'service';productType?:'product'|'service'|'combo';unitCode?:string;saleEnabled:boolean;purchaseEnabled?:boolean;status:'active'|'archived';stockOnHandMilli?:number;priceRules?:PosPriceRuleView[];lots?:PosLotInputView[];attributeValues?:PosVariantAttributeInputView[];attributeLines?:PosTemplateAttributeLineInputView[];generateVariants?:boolean;optionalProductIds?:string[];comboComponents?:PosComboComponentInputView[]}
export interface PosResolvedVariantView {product:PosLoadDataView['products'][number];customAttributes:Array<{attributeId:string;attributeName:string;valueId:string;valueName:string;priceExtraMinor:number;color?:string}>;customPriceExtraMinor:number}
export interface PosLoadDataView { organizationId:string; companyId:string; entitled:boolean; config?:{id:string;name:string;restaurant:boolean;taxIncluded:boolean;cashRoundingIncrementMinor:number;cashRoundingMethod:'nearest'|'up'|'down';settings:PosConfigSettingsView};activeOperator?:PosOperatorView;operators:PosOperatorView[]; session?:{id:string;state:'opening'|'opened'|'closing'|'closed';openedAt?:string;openingCashMinor:number;openingNote?:string;expectedCashMinor:number;paidOrderCount:number;draftOrderCount:number;grossSalesMinor:number;taxSalesMinor:number;discountSalesMinor:number;draftSalesMinor:number;refundMinor:number;paymentsByMethod:Array<{paymentMethodId:string;name:string;amountMinor:number}>;salesByProduct:Array<{productVariantId:string;name:string;quantity:number;amountMinor:number}>;salesByCategory:Array<{category:string;quantity:number;amountMinor:number}>;cashMoves:Array<{id:string;direction:'in'|'out';amountMinor:number;reason:string;createdAt:string}>}; products:Array<{id:string;templateId:string;name:string;description:string;kind:'product'|'service';productType:'product'|'service'|'combo';unitCode:string;saleEnabled:boolean;purchaseEnabled:boolean;status:'active'|'archived';sku?:string;barcode?:string;categoryId?:string;taxId?:string;category:string;priceMinor:number;taxBasisPoints:number;stockOnHandMilli:number;priceRules:Array<PosPriceRuleView&{priceListName:string}>;media:Array<{id:string;mimeType:string;bytes:number;position:number;altText:string}>;attributes:Array<{attributeId:string;attributeName:string;valueId:string;valueName:string;priceExtraMinor:number;color?:string}>;attributeLines:PosTemplateAttributeLineInputView[];optionalProductIds:string[];comboComponents:Array<{id:string;name:string;categoryId?:string;quantity:number;minChoices:number;maxChoices:number;items:Array<{id:string;productVariantId:string;extraPriceMinor:number;maxFreeQuantity:number;sequence:number}>}>;lots:Array<{id:string;name:string;tracking:'lot'|'serial';quantityMilli:number;expirationDate?:string}>}>;catalog:{categories:PosCatalogCategoryView[];taxes:PosCatalogTaxView[]};partners:PosPartnerView[];pricelists:Array<{id:string;name:string;currencyCode:string}>;fiscalPositions:Array<{id:string;name:string;mappings:Array<{sourceRateBasisPoints:number;destinationRateBasisPoints:number}>}>; floors:Array<{id:string;name:string;backgroundColor?:string;tables:Array<{id:string;name:string;seats:number;shape:'square'|'round';color?:string;width:number;height:number;positionX:number;positionY:number;parentTableId?:string}>}>;paymentMethods:Array<{id:string;name:string;methodType:'cash'|'bank'|'customer_account'|'terminal';requiresTerminal:boolean;splitTransactions:boolean}>;preparationPrinters:Array<{id:string;name:string;printerType:'desktop'|'network'|'iot';categoryIds:string[]}>; orders:PosOrderView[];tickets:PosOrderView[] }
export interface PosOrderView { id:string;uuid:string;organizationId:string;companyId:string;sessionId:string;tableId?:string;operatorId?:string;operatorName?:string;state:'draft'|'paid'|'cancelled';createdAt?:string;untaxedMinor:number;taxMinor:number;totalMinor:number;metadata:PosOrderMetadataView;lines:Array<PosOrderLineInputView&{id:string;description:string;unitPriceMinor:number;subtotalMinor:number;taxMinor:number;totalMinor:number}> }
...
export interface DispatchFileView {
  id: string; name: string; mimeType: string; bytes: number; role: "pdf" | "xml";
  source: "dispatch" | "fiscal";
}
...
export interface DispatchDocumentView {
  ...
  /** PDF/XML retained from the source or generated by the fiscal provider. */
  files?: DispatchFileView[];
}
...
export interface DispatchFileContentView { name: string; mimeType: string; bytes: Uint8Array }
...
export interface AuthenticatedApi extends RpcTarget {
  ...
  posLoginOperator(organizationId:string,companyId:string,operatorId:string,pin?:string):Promise<PosOperatorView>;
  ...
  posSavePreparationPrinter(organizationId:string,companyId:string,input:{id?:string;name:string;printerType:'desktop'|'network'|'iot';categoryIds:string[];status?:'active'|'archived'}):Promise<PosLoadDataView['preparationPrinters'][number]>;
  posCreatePartner(organizationId:string,companyId:string,input:PosPartnerInputView):Promise<PosPartnerView>;
  posUpdatePartner(organizationId:string,companyId:string,partnerId:string,input:PosPartnerInputView):Promise<PosPartnerView>;
  ...
  posSaveProductMedia(organizationId:string,companyId:string,productVariantId:string,input:{name:string;mimeType:string;bytes:Uint8Array;altText?:string}):Promise<PosLoadDataView['products'][number]['media'][number]>;
  posReadProductMedia(organizationId:string,companyId:string,mediaId:string):Promise<{mimeType:string;bytes:Uint8Array}>;
  posDeleteProductMedia(organizationId:string,companyId:string,mediaId:string):Promise<void>;
  posResolveProductVariant(organizationId:string,companyId:string,templateId:string,valueIds:string[]):Promise<PosResolvedVariantView>;
  posOpenSession(organizationId:string,companyId:string,openingCashMinor:number,openingNote?:string,denominations?:PosCashDenominationView[]):Promise<PosLoadDataView['session']>;
  ...
  posSetPreparationState(organizationId:string,companyId:string,orderId:string,state:'preparing'|'ready'|'served'):Promise<PosOrderView>;
  ...
  posCloseSession(organizationId:string,companyId:string,sessionId:string,countedCashMinor:number,closingNote?:string,paymentCounts?:PosSessionPaymentCountView[],denominations?:PosCashDenominationView[]):Promise<{expectedCashMinor:number;countedCashMinor:number;differenceMinor:number;paymentDifferences:Array<{paymentMethodId:string;expectedMinor:number;countedMinor:number;differenceMinor:number}>}>;
  ...
  readDispatchFile(organizationId: string, companyId: string, dispatchDocumentId: string, role: "pdf" | "xml"): Promise<DispatchFileContentView>;
