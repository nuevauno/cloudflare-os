...
export interface PosCatalogProductInputView {id?:string;templateId?:string;name:string;description?:string;sku?:string;barcode?:string;categoryId?:string;priceMinor:number;taxId?:string;kind:'product'|'service';productType?:'product'|'service'|'combo';unitCode?:string;saleEnabled:boolean;purchaseEnabled?:boolean;status:'active'|'archived';attributeValues?:PosVariantAttributeInputView[];attributeLines?:PosTemplateAttributeLineInputView[];generateVariants?:boolean;optionalProductIds?:string[];comboComponents?:PosComboComponentInputView[]}
