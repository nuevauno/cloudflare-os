...
interface PlatformCoreBinding {
  ...
  posCloseSession(actorSubject:string,organizationId:string,companyId:string,sessionId:string,countedCashMinor:number):Promise<{expectedCashMinor:number;countedCashMinor:number;differenceMinor:number}>;
...
class AuthenticatedApiImpl extends RpcTarget implements AuthenticatedApi {
  ...
  async posCloseSession(organizationId:string,companyId:string,sessionId:string,countedCashMinor:number){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posCloseSession(await this.#businessSubject(organizationId,companyId),organizationId,companyId,sessionId,countedCashMinor)}
