...
interface PlatformCoreBinding {
  ...
  posPayOrder(input:{actorSubject:string;organizationId:string;companyId:string;orderId:string;tenderedMinor:number;paymentMethodId?:string;idempotencyKey:string}):Promise<{order:import('@gadgets/workshop-shared/api').PosOrderView;payment:{id:string;tenderedMinor:number;changeMinor:number}}>;
...
class AuthenticatedApiImpl extends RpcTarget implements AuthenticatedApi {
  ...
  async posPayOrder(input:{organizationId:string;companyId:string;orderId:string;tenderedMinor:number;paymentMethodId?:string;requestId:string}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posPayOrder({organizationId:input.organizationId,companyId:input.companyId,orderId:input.orderId,tenderedMinor:input.tenderedMinor,...(input.paymentMethodId?{paymentMethodId:input.paymentMethodId}:{}),actorSubject:await this.#businessSubject(input.organizationId,input.companyId),idempotencyKey:`pos:pay:${this.#userId.name}:${input.requestId}`})}
