...
interface PlatformCoreBinding {
  ...
  posCreatePartner(actorSubject:string,organizationId:string,companyId:string,input:{displayName:string;email?:string;phone?:string;taxIdentifier?:string}):Promise<import('@gadgets/workshop-shared/api').PosLoadDataView['partners'][number]>;
...
class AuthenticatedApiImpl extends RpcTarget implements AuthenticatedApi {
  ...
  async posCreatePartner(organizationId:string,companyId:string,input:{displayName:string;email?:string;phone?:string;taxIdentifier?:string}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posCreatePartner(await this.#businessSubject(organizationId,companyId),organizationId,companyId,input)}
