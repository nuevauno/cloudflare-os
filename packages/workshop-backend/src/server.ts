...
interface PlatformCoreBinding {
  ...
  posLoginOperator(actorSubject:string,organizationId:string,companyId:string,operatorId:string,pin?:string):Promise<import('@gadgets/workshop-shared/api').PosOperatorView>;
  posLogoutOperator(actorSubject:string,organizationId:string,companyId:string):Promise<void>;
  posSaveOperator(actorSubject:string,organizationId:string,companyId:string,input:{id?:string;displayName:string;role:import('@gadgets/workshop-shared/api').PosOperatorRoleView;pin?:string;status?:'active'|'archived'}):Promise<import('@gadgets/workshop-shared/api').PosOperatorView>;
  posUpdateSettings(actorSubject:string,organizationId:string,companyId:string,settings:import('@gadgets/workshop-shared/api').PosConfigSettingsView):Promise<NonNullable<import('@gadgets/workshop-shared/api').PosLoadDataView['config']>>;
  posSavePaymentMethod(actorSubject:string,organizationId:string,companyId:string,input:{id?:string;name:string;methodType:'cash'|'bank'|'customer_account'|'terminal';requiresTerminal:boolean;splitTransactions:boolean;status?:'active'|'archived'}):Promise<import('@gadgets/workshop-shared/api').PosLoadDataView['paymentMethods'][number]>;
  posSavePreparationPrinter(actorSubject:string,organizationId:string,companyId:string,input:{id?:string;name:string;printerType:'desktop'|'network'|'iot';categoryIds:string[];status?:'active'|'archived'}):Promise<import('@gadgets/workshop-shared/api').PosLoadDataView['preparationPrinters'][number]>;
  posCreatePartner(actorSubject:string,organizationId:string,companyId:string,input:import('@gadgets/workshop-shared/api').PosPartnerInputView):Promise<import('@gadgets/workshop-shared/api').PosPartnerView>;
  posUpdatePartner(actorSubject:string,organizationId:string,companyId:string,partnerId:string,input:import('@gadgets/workshop-shared/api').PosPartnerInputView):Promise<import('@gadgets/workshop-shared/api').PosPartnerView>;
  posSaveCategory(actorSubject:string,organizationId:string,companyId:string,input:{id?:string;name:string;parentId?:string;status?:'active'|'archived'}):Promise<import('@gadgets/workshop-shared/api').PosCatalogCategoryView>;
  posSaveProduct(actorSubject:string,organizationId:string,companyId:string,input:import('@gadgets/workshop-shared/api').PosCatalogProductInputView):Promise<import('@gadgets/workshop-shared/api').PosLoadDataView['products'][number]>;
  posSaveProductMedia(actorSubject:string,organizationId:string,companyId:string,productVariantId:string,input:{name:string;mimeType:string;bytes:Uint8Array;altText?:string}):Promise<import('@gadgets/workshop-shared/api').PosLoadDataView['products'][number]['media'][number]>;
  posReadProductMedia(actorSubject:string,organizationId:string,companyId:string,mediaId:string):Promise<{mimeType:string;bytes:Uint8Array}>;
  posDeleteProductMedia(actorSubject:string,organizationId:string,companyId:string,mediaId:string):Promise<void>;
  posResolveProductVariant(actorSubject:string,organizationId:string,companyId:string,templateId:string,valueIds:string[]):Promise<import('@gadgets/workshop-shared/api').PosResolvedVariantView>;
  posOpenSession(actorSubject:string,organizationId:string,companyId:string,openingCashMinor:number,openingNote?:string,denominations?:import('@gadgets/workshop-shared/api').PosCashDenominationView[]):Promise<import('@gadgets/workshop-shared/api').PosLoadDataView['session']>;
  posSyncOrder(input:{actorSubject:string;organizationId:string;companyId:string;sessionId:string;tableId?:string;uuid:string;metadata?:Partial<import('@gadgets/workshop-shared/api').PosOrderMetadataView>;lines:import('@gadgets/workshop-shared/api').PosOrderLineInputView[]}):Promise<import('@gadgets/workshop-shared/api').PosOrderView>;
  posPayOrder(input:{actorSubject:string;organizationId:string;companyId:string;orderId:string;tenderedMinor?:number;paymentMethodId?:string;payments?:Array<{paymentMethodId:string;amountMinor:number;tenderedMinor?:number;terminalReference?:string}>;idempotencyKey:string}):Promise<{order:import('@gadgets/workshop-shared/api').PosOrderView;payment:{id:string;paymentMethodId:string;amountMinor:number;tenderedMinor:number;changeMinor:number};payments:Array<{id:string;paymentMethodId:string;amountMinor:number;tenderedMinor:number;changeMinor:number}>}>;
  posCashMove(input:{actorSubject:string;organizationId:string;companyId:string;sessionId:string;direction:'in'|'out';amountMinor:number;reason:string;idempotencyKey:string}):Promise<void>;
  posSplitOrder(input:{actorSubject:string;organizationId:string;companyId:string;orderId:string;lineQuantities:Array<{lineId:string;quantity:number}>;idempotencyKey:string}):Promise<{source:import('@gadgets/workshop-shared/api').PosOrderView;split:import('@gadgets/workshop-shared/api').PosOrderView}>;
  posRefundOrder(input:{actorSubject:string;organizationId:string;companyId:string;orderId:string;lines:Array<{lineId:string;quantity:number}>;idempotencyKey:string}):Promise<import('@gadgets/workshop-shared/api').PosOrderView>;
  posSendToPreparation(actorSubject:string,organizationId:string,companyId:string,orderId:string):Promise<import('@gadgets/workshop-shared/api').PosOrderView>;
  posSetPreparationState(actorSubject:string,organizationId:string,companyId:string,orderId:string,state:'preparing'|'ready'|'served'):Promise<import('@gadgets/workshop-shared/api').PosOrderView>;
  posTransferOrder(actorSubject:string,organizationId:string,companyId:string,orderId:string,tableId:string):Promise<import('@gadgets/workshop-shared/api').PosOrderView>;
  posMergeOrders(actorSubject:string,organizationId:string,companyId:string,sourceOrderId:string,targetOrderId:string):Promise<import('@gadgets/workshop-shared/api').PosOrderView>;
  posCreateTable(actorSubject:string,organizationId:string,companyId:string,floorId:string,name:string,seats:number):Promise<import('@gadgets/workshop-shared/api').PosLoadDataView['floors'][number]['tables'][number]>;
  posUpdateTable(actorSubject:string,organizationId:string,companyId:string,tableId:string,changes:Partial<Pick<import('@gadgets/workshop-shared/api').PosLoadDataView['floors'][number]['tables'][number],'name'|'seats'|'shape'|'color'|'width'|'height'|'positionX'|'positionY'>>):Promise<import('@gadgets/workshop-shared/api').PosLoadDataView['floors'][number]['tables'][number]>;
  posDeleteTable(actorSubject:string,organizationId:string,companyId:string,tableId:string):Promise<void>;
  posCancelOrder(actorSubject:string,organizationId:string,companyId:string,orderId:string):Promise<void>;
  posCloseSession(actorSubject:string,organizationId:string,companyId:string,sessionId:string,countedCashMinor:number,closingNote?:string,paymentCounts?:import('@gadgets/workshop-shared/api').PosSessionPaymentCountView[],denominations?:import('@gadgets/workshop-shared/api').PosCashDenominationView[]):Promise<{expectedCashMinor:number;countedCashMinor:number;differenceMinor:number;paymentDifferences:Array<{paymentMethodId:string;expectedMinor:number;countedMinor:number;differenceMinor:number}>}>;
...
class AuthenticatedApiImpl extends RpcTarget implements AuthenticatedApi {
  ...
  async posLoginOperator(organizationId:string,companyId:string,operatorId:string,pin?:string){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posLoginOperator(await this.#businessSubject(organizationId,companyId),organizationId,companyId,operatorId,pin)}
  async posLogoutOperator(organizationId:string,companyId:string){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posLogoutOperator(await this.#businessSubject(organizationId,companyId),organizationId,companyId)}
  async posSaveOperator(organizationId:string,companyId:string,input:{id?:string;displayName:string;role:import('@gadgets/workshop-shared/api').PosOperatorRoleView;pin?:string;status?:'active'|'archived'}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSaveOperator(await this.#businessSubject(organizationId,companyId),organizationId,companyId,input)}
  async posUpdateSettings(organizationId:string,companyId:string,settings:import('@gadgets/workshop-shared/api').PosConfigSettingsView){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posUpdateSettings(await this.#businessSubject(organizationId,companyId),organizationId,companyId,settings)}
  async posSavePaymentMethod(organizationId:string,companyId:string,input:{id?:string;name:string;methodType:'cash'|'bank'|'customer_account'|'terminal';requiresTerminal:boolean;splitTransactions:boolean;status?:'active'|'archived'}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSavePaymentMethod(await this.#businessSubject(organizationId,companyId),organizationId,companyId,input)}
  async posSavePreparationPrinter(organizationId:string,companyId:string,input:{id?:string;name:string;printerType:'desktop'|'network'|'iot';categoryIds:string[];status?:'active'|'archived'}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSavePreparationPrinter(await this.#businessSubject(organizationId,companyId),organizationId,companyId,input)}
  async posCreatePartner(organizationId:string,companyId:string,input:import('@gadgets/workshop-shared/api').PosPartnerInputView){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posCreatePartner(await this.#businessSubject(organizationId,companyId),organizationId,companyId,input)}
  async posUpdatePartner(organizationId:string,companyId:string,partnerId:string,input:import('@gadgets/workshop-shared/api').PosPartnerInputView){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posUpdatePartner(await this.#businessSubject(organizationId,companyId),organizationId,companyId,partnerId,input)}
  async posSaveCategory(organizationId:string,companyId:string,input:{id?:string;name:string;parentId?:string;status?:'active'|'archived'}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSaveCategory(await this.#businessSubject(organizationId,companyId),organizationId,companyId,input)}
  async posSaveProduct(organizationId:string,companyId:string,input:import('@gadgets/workshop-shared/api').PosCatalogProductInputView){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSaveProduct(await this.#businessSubject(organizationId,companyId),organizationId,companyId,input)}
  async posSaveProductMedia(organizationId:string,companyId:string,productVariantId:string,input:{name:string;mimeType:string;bytes:Uint8Array;altText?:string}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSaveProductMedia(await this.#businessSubject(organizationId,companyId),organizationId,companyId,productVariantId,input)}
  async posReadProductMedia(organizationId:string,companyId:string,mediaId:string){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posReadProductMedia(await this.#businessSubject(organizationId,companyId),organizationId,companyId,mediaId)}
  async posDeleteProductMedia(organizationId:string,companyId:string,mediaId:string){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posDeleteProductMedia(await this.#businessSubject(organizationId,companyId),organizationId,companyId,mediaId)}
  async posResolveProductVariant(organizationId:string,companyId:string,templateId:string,valueIds:string[]){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posResolveProductVariant(await this.#businessSubject(organizationId,companyId),organizationId,companyId,templateId,valueIds)}
  async posOpenSession(organizationId:string,companyId:string,openingCashMinor:number,openingNote?:string,denominations?:import('@gadgets/workshop-shared/api').PosCashDenominationView[]){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posOpenSession(await this.#businessSubject(organizationId,companyId),organizationId,companyId,openingCashMinor,openingNote,denominations)}
  async posSyncOrder(input:{organizationId:string;companyId:string;sessionId:string;tableId?:string;uuid:string;metadata?:Partial<import('@gadgets/workshop-shared/api').PosOrderMetadataView>;lines:import('@gadgets/workshop-shared/api').PosOrderLineInputView[]}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSyncOrder({...input,actorSubject:await this.#businessSubject(input.organizationId,input.companyId)})}
  async posPayOrder(input:{organizationId:string;companyId:string;orderId:string;tenderedMinor?:number;paymentMethodId?:string;payments?:Array<{paymentMethodId:string;amountMinor:number;tenderedMinor?:number;terminalReference?:string}>;requestId:string}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posPayOrder({organizationId:input.organizationId,companyId:input.companyId,orderId:input.orderId,...(input.tenderedMinor===undefined?{}:{tenderedMinor:input.tenderedMinor}),...(input.paymentMethodId?{paymentMethodId:input.paymentMethodId}:{}),...(input.payments?{payments:input.payments}:{}),actorSubject:await this.#businessSubject(input.organizationId,input.companyId),idempotencyKey:`pos:pay:${this.#userId.name}:${input.requestId}`})}
  async posCashMove(input:{organizationId:string;companyId:string;sessionId:string;direction:'in'|'out';amountMinor:number;reason:string;requestId:string}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posCashMove({...input,actorSubject:await this.#businessSubject(input.organizationId,input.companyId),idempotencyKey:`pos:cash:${this.#userId.name}:${input.requestId}`})}
  async posSplitOrder(input:{organizationId:string;companyId:string;orderId:string;lineQuantities:Array<{lineId:string;quantity:number}>;requestId:string}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSplitOrder({...input,actorSubject:await this.#businessSubject(input.organizationId,input.companyId),idempotencyKey:`pos:split:${this.#userId.name}:${input.requestId}`})}
  async posRefundOrder(input:{organizationId:string;companyId:string;orderId:string;lines:Array<{lineId:string;quantity:number}>;requestId:string}){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posRefundOrder({...input,actorSubject:await this.#businessSubject(input.organizationId,input.companyId),idempotencyKey:`pos:refund:${this.#userId.name}:${input.requestId}`})}
  async posSendToPreparation(organizationId:string,companyId:string,orderId:string){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSendToPreparation(await this.#businessSubject(organizationId,companyId),organizationId,companyId,orderId)}
  async posSetPreparationState(organizationId:string,companyId:string,orderId:string,state:'preparing'|'ready'|'served'){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posSetPreparationState(await this.#businessSubject(organizationId,companyId),organizationId,companyId,orderId,state)}
  async posTransferOrder(organizationId:string,companyId:string,orderId:string,tableId:string){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posTransferOrder(await this.#businessSubject(organizationId,companyId),organizationId,companyId,orderId,tableId)}
  async posMergeOrders(organizationId:string,companyId:string,sourceOrderId:string,targetOrderId:string){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posMergeOrders(await this.#businessSubject(organizationId,companyId),organizationId,companyId,sourceOrderId,targetOrderId)}
  async posCreateTable(organizationId:string,companyId:string,floorId:string,name:string,seats:number){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posCreateTable(await this.#businessSubject(organizationId,companyId),organizationId,companyId,floorId,name,seats)}
  async posUpdateTable(organizationId:string,companyId:string,tableId:string,changes:Partial<Pick<import('@gadgets/workshop-shared/api').PosLoadDataView['floors'][number]['tables'][number],'name'|'seats'|'shape'|'color'|'width'|'height'|'positionX'|'positionY'>>){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posUpdateTable(await this.#businessSubject(organizationId,companyId),organizationId,companyId,tableId,changes)}
  async posDeleteTable(organizationId:string,companyId:string,tableId:string){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posDeleteTable(await this.#businessSubject(organizationId,companyId),organizationId,companyId,tableId)}
  ...
  async posCloseSession(organizationId:string,companyId:string,sessionId:string,countedCashMinor:number,closingNote?:string,paymentCounts?:import('@gadgets/workshop-shared/api').PosSessionPaymentCountView[],denominations?:import('@gadgets/workshop-shared/api').PosCashDenominationView[]){if(!this.env.PLATFORM_CORE)throw new Error('business_core_unavailable');return this.env.PLATFORM_CORE.posCloseSession(await this.#businessSubject(organizationId,companyId),organizationId,companyId,sessionId,countedCashMinor,closingNote,paymentCounts,denominations)}
  ...
  async recordCommercialPayment(input: import('@gadgets/workshop-shared/api').RecordCommercialPaymentRequest): Promise<import('@gadgets/workshop-shared/api').RecordCommercialPaymentResultView> {
    ...
    return {
      ...
      residualMinor: result.residualMinor,
