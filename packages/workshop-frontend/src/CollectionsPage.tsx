...
export default function CollectionsPage() {
  ...
  return <div className="mx-auto w-full max-w-6xl px-5 py-8 md:px-8 md:py-10">
    ...
    {sample && <div className="mb-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{cards.map(([label, value]) => <div key={label} className="border border-kumo-line bg-kumo-elevated p-4"><span className="text-xs text-kumo-subtle">{label}</span><span className="mt-2 block text-xl text-kumo-default">{formatMoney(sample, value, language)}</span></div>)}</div>}
    <div className="mb-4 flex flex-wrap gap-2">{(['all', 'overdue', 'dueSoon', 'current', 'paid'] as Filter[]).map((item) => <button key={item} type="button" onClick={() => setFilter(item)} className={`cursor-pointer border px-3 py-2 text-xs ${filter === item ? 'border-[#FE4A23] bg-[#FE4A23] text-white' : 'border-kumo-line bg-kumo-elevated text-kumo-default'}`}>{t(`collections.${item}`)}</button>)}</div>
    <div className="overflow-hidden border border-kumo-line bg-kumo-elevated">
      {loading ? <p className="p-6 text-sm text-kumo-subtle">{t('common.loading')}</p> : failed ? <p className="p-6 text-sm text-kumo-danger">{t('collections.error')}</p> : visible.length ? visible.map((document) => <article key={document.id} className="grid gap-3 border-b border-kumo-line p-4 last:border-b-0 sm:grid-cols-[40px_1fr_auto_auto] sm:items-center">
        <span className="flex h-9 w-9 items-center justify-center border border-kumo-line bg-kumo-base"><NuevaunoIcon name="nuevauno_billing" size={17} /></span>
        ...
        {document.residualMinor > 0 ? <button type="button" onClick={() => openPayment(document)} className="cursor-pointer bg-[#FE4A23] px-3 py-2 text-xs text-white">{t('collections.record')}</button> : <span className="text-xs text-kumo-subtle">{t('collections.paid')}</span>}
    ...
    {paying && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/35 p-4 sm:items-center" role="dialog" aria-modal="true">
      <div className="w-full max-w-md border border-kumo-line bg-kumo-elevated p-5 shadow-xl">
        ...
        <div className="mt-5 grid gap-4">
          <label className="grid gap-1 text-xs text-kumo-subtle">{t('collections.amount')}<input value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" className="border border-kumo-line bg-kumo-base px-3 py-2 text-sm text-kumo-default" /></label>
          <label className="grid gap-1 text-xs text-kumo-subtle">{t('collections.date')}<input type="date" value={paidOn} onChange={(event) => setPaidOn(event.target.value)} className="border border-kumo-line bg-kumo-base px-3 py-2 text-sm text-kumo-default" /></label>
          <label className="grid gap-1 text-xs text-kumo-subtle">{t('collections.method')}<select value={method} onChange={(event) => setMethod(event.target.value as Method)} className="border border-kumo-line bg-kumo-base px-3 py-2 text-sm text-kumo-default">{(['bank_transfer', 'card', 'cash', 'check', 'other'] as Method[]).map((item) => <option key={item} value={item}>{t(`collections.method.${item}`)}</option>)}</select></label>
          <label className="grid gap-1 text-xs text-kumo-subtle">{t('collections.reference')}<input value={reference} onChange={(event) => setReference(event.target.value)} className="border border-kumo-line bg-kumo-base px-3 py-2 text-sm text-kumo-default" /></label>
        </div>
        {paymentFailed && <p className="mt-3 text-xs text-kumo-danger">{t('collections.paymentError')}</p>}
        <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setPaying(null)} className="cursor-pointer border border-kumo-line px-3 py-2 text-xs text-kumo-default">{t('collections.cancel')}</button><button type="button" disabled={saving} onClick={savePayment} className="cursor-pointer bg-[#FE4A23] px-3 py-2 text-xs text-white disabled:opacity-50">{saving ? t('collections.saving') : t('collections.save')}</button></div>
      </div>
    </div>}
  </div>
}
