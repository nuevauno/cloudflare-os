...
const POS_SETTING_SECTIONS:PosSettingSection[]=[
  ...
  {title:"Pago",items:[
    ...
    {key:"pos_nuevauno_suggested_tip_scope",label:"Aplicar propina en",help:"Define en qué tipos de pedido se ofrece la propina.",kind:"select",options:[{value:"dinein",label:"Consumo en mesa"},{value:"takeaway",label:"Para llevar"},{value:"all",label:"Todos los pedidos"}]},
...
export default function PosPage() {
  ...
  const [cart, setCart] = useState<Record<string, number>>({}),
    ...
    [editingPartnerId,setEditingPartnerId]=useState(""),
    [actionsOpen,setActionsOpen]=useState(false),
    [splitQuantities,setSplitQuantities]=useState<Record<string,number>>({}),
    [partnerDraft, setPartnerDraft] = useState<PosPartnerInputView>({kind:"person",displayName:"",email:"",phone:"",taxIdentifier:"",street:"",street2:"",city:"",regionCode:"",postalCode:"",countryCode:"CL",website:""}),
  ...
  const emptyPartnerDraft:PosPartnerInputView={kind:"person",displayName:"",email:"",phone:"",taxIdentifier:"",street:"",street2:"",city:"",regionCode:"",postalCode:"",countryCode:"CL",website:""};
  const openPartnerCreate=()=>{setEditingPartnerId("");setPartnerDraft(emptyPartnerDraft);setPartnerCreate(true);setPartnerDialog(true)};
  const openPartnerEdit=(partner:PosPartnerView)=>{setEditingPartnerId(partner.id);setPartnerDraft({...emptyPartnerDraft,...partner});setPartnerCreate(true);setPartnerDialog(true)};
  const partnerDialogContent=partnerDialog&&<div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4"><section role="dialog" aria-modal="true" aria-label="Elegir cliente" className="flex h-[82vh] w-full max-w-5xl flex-col rounded-xl bg-kumo-elevated shadow-xl">
    <header className="flex items-center gap-3 border-b border-kumo-line p-4"><button onClick={()=>partnerCreate?setPartnerCreate(false):openPartnerCreate()} className="rounded-xl bg-[#FE4A23] px-5 py-3 text-white">{partnerCreate?"Volver":"Crear"}</button><h2 className="text-xl font-normal">{partnerCreate?(editingPartnerId?"Editar cliente":"Nuevo cliente"):"Elige un cliente"}</h2>{!partnerCreate&&<input autoFocus value={partnerSearch} onChange={event=>setPartnerSearch(event.target.value)} placeholder="Buscar clientes…" className="ml-auto w-72 rounded-xl border border-kumo-line bg-kumo-base p-3"/>}</header>
    {partnerCreate?<form onSubmit={async event=>{event.preventDefault();const saved=editingPartnerId?await authenticatedApi.posUpdatePartner(scope.organizationId,scope.companyId,editingPartnerId,partnerDraft):await authenticatedApi.posCreatePartner(scope.organizationId,scope.companyId,partnerDraft);setPartnerId(saved.id);setPartnerDialog(false);setPartnerCreate(false);setEditingPartnerId("");setPartnerDraft(emptyPartnerDraft);await refresh()}} className="grid flex-1 content-start gap-4 overflow-y-auto p-6 md:grid-cols-2">
      <label>Tipo<select value={partnerDraft.kind} onChange={event=>setPartnerDraft(current=>({...current,kind:event.target.value as 'person'|'company'}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"><option value="person">Persona</option><option value="company">Empresa</option></select></label>
      <label>Nombre<input required autoFocus value={partnerDraft.displayName} onChange={event=>setPartnerDraft(current=>({...current,displayName:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>RUT<input value={partnerDraft.taxIdentifier??""} onChange={event=>setPartnerDraft(current=>({...current,taxIdentifier:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>Correo<input type="email" value={partnerDraft.email??""} onChange={event=>setPartnerDraft(current=>({...current,email:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>Teléfono<input value={partnerDraft.phone??""} onChange={event=>setPartnerDraft(current=>({...current,phone:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>Dirección<input value={partnerDraft.street??""} onChange={event=>setPartnerDraft(current=>({...current,street:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>Complemento<input value={partnerDraft.street2??""} onChange={event=>setPartnerDraft(current=>({...current,street2:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>Ciudad / comuna<input value={partnerDraft.city??""} onChange={event=>setPartnerDraft(current=>({...current,city:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>Región<input value={partnerDraft.regionCode??""} onChange={event=>setPartnerDraft(current=>({...current,regionCode:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>Código postal<input value={partnerDraft.postalCode??""} onChange={event=>setPartnerDraft(current=>({...current,postalCode:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>País<input maxLength={2} value={partnerDraft.countryCode??"CL"} onChange={event=>setPartnerDraft(current=>({...current,countryCode:event.target.value.toUpperCase()}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <label>Sitio web<input type="url" value={partnerDraft.website??""} onChange={event=>setPartnerDraft(current=>({...current,website:event.target.value}))} className="mt-2 w-full rounded-xl border border-kumo-line bg-kumo-base p-3"/></label>
      <button className="rounded-xl bg-[#FE4A23] p-4 text-white md:col-span-2">{editingPartnerId?"Guardar cambios":"Guardar cliente"}</button>
    </form>:<div className="flex-1 overflow-y-auto"><button onClick={()=>{setPartnerId("");setPartnerDialog(false)}} className="grid w-full grid-cols-[1fr_auto] border-b border-kumo-line p-5 text-left"><span>Consumidor final</span><span>Seleccionar</span></button>{data.partners.filter(partner=>[partner.displayName,partner.email,partner.phone,partner.taxIdentifier,partner.street,partner.city].filter(Boolean).join(" ").toLowerCase().includes(partnerSearch.toLowerCase())).map(partner=><div key={partner.id} className={`grid grid-cols-[1fr_auto] items-center border-b border-kumo-line ${partner.id===partnerId?"bg-orange-50":""}`}><button onClick={()=>{setPartnerId(partner.id);setPartnerDialog(false)}} className="grid grid-cols-3 gap-3 p-5 text-left"><span>{partner.displayName}<small className="block text-kumo-subtle">{partner.taxIdentifier}</small></span><span>{partner.email}<small className="block text-kumo-subtle">{partner.phone}</small></span><span>{[partner.street,partner.city].filter(Boolean).join(", ")}</span></button><button onClick={()=>openPartnerEdit(partner)} className="m-3 rounded-xl border border-kumo-line px-4 py-3">Editar</button></div>)}</div>}
  ...
  if (screen === "payment") {
    const tipScope=String(data.config?.settings.pos_nuevauno_suggested_tip_scope??"dinein"),
      tipEnabled = Boolean(data.config?.settings.pos_iface_tipproduct) && (tipScope==="all"||(tipScope==="dinein"&&Boolean(table)&&!takeaway)||(tipScope==="takeaway"&&takeaway)),
      ...
      <main className="grid min-h-full grid-cols-[minmax(260px,340px)_1fr_minmax(300px,440px)] bg-kumo-base">
        ...
        <aside className="flex min-h-full flex-col border-l border-kumo-line bg-kumo-elevated p-4">
          ...
          <div className="mt-auto space-y-2">
            ...
            {tipEnabled&&<button onClick={async() => {const pct=Math.max(0,Number(data.config?.settings.pos_nuevauno_suggested_tip_pct??10)||10),suggested=Math.round((rawTotal-tipMinor)*pct/100),value=Number(await requestDialog({kind:"number",title:"Propina",label:`Sugerida ${pct}% · ${money(suggested)}`,value:tipMinor||suggested,min:0}));if(Number.isSafeInteger(value)&&value>=0){setTipMinor(value);setPaymentBucket(value>0?"tip":"products")}}} className={`flex w-full justify-between rounded-xl border p-4 ${tipMinor ? "border-[#FE4A23] bg-orange-50" : "border-kumo-line"}`}><span>Propina</span><span>{tipMinor ? money(tipMinor) : "Agregar"}</span></button>}
