...
describe('loadCertificates', () => {
  it('uses the active authorized company and never accepts a browser-provided scope', async () => {
    ...
    const listCertificates = vi.fn().mockResolvedValue({ organizationId: 'org_rng', companyId: 'cmp_rng', certificates: [] })
    const getBusinessSession = vi.fn().mockResolvedValue(session)
