...
describe('fiscal company scope', () => {
  it('reads only the active authorized company', async () => {
    const listFiscalDocuments = vi.fn<() => Promise<{ organizationId: string; companyId: string; documents: never[] }>>().mockResolvedValue({ organizationId: 'org_rng', companyId: 'cmp_rng', documents: [] })
    const api = { getBusinessSession: vi.fn<() => Promise<never>>(), listFiscalDocuments }
